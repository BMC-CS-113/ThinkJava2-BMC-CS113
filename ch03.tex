...
\java{System} is a class\footnote{Remember that so far a class is a collection of related methods.} that provides methods related to the ``system'', or environment, where programs run.
...
In the file {\it Echo.java}, remove the import statement, compile the file, and see if you can
get this ``cannot find symbol'' error message from the compiler.
...
By convention, names for constants are all uppercase, with the underscore character (\java{_}) between words. The variables are considered {\bf immutable}, meaning that their values cannot change after
being initalized.
...
What error message do you see when trying to compile this code?
...
\term{immutable}
Variables whose values cannot change after being initalized
