...
In this example, the \java{newLine} and \java{main} methods are both \java{public}, which means they can be {\bf invoked} (or ``called'') from other classes (or even the command line for the case of \java{main}).
...
Programs always begin at the first statement of \java{main}, regardless of where it is in the source file.\footnote{Though by convention we put the \java{main} method as the last method in our source file.}
