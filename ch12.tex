...
This version of \java{search} uses the algorithm in Section~\ref{traversal}, which is called {\bf linear search}:
...
If the cards in the array are not in order, there is no way to search faster than linear search.
We have to look at every card, because otherwise we can't be certain the card we want is not there.
We call this algorithm linear search since the number of spots in the array we need to look at \textit{increases linearly} as the size of the array increases. In other words, if the array has a length of \textit{n}, then in the worst case scenario we will have to look at all \textit{n} spots in the array (this worst case scenario is when the element we are searching for is not in the array). 
In turn, we refer to the run-time of linear search as \textit{O(n)} (pronounced as Big-Oh of n). The Big-Oh notation is a common way in Computer Science to indicate how long an algorithm will take to run in the worst case scenario.

But if the cards are in order, we can use better algorithms.
Linear search is relatively inefficient, especially for large arrays.
...
This algorithm is much faster than linear search, because it rules out half of the remaining words each time you make a comparison.
...
So we might have to look at 5 or 6 cards, as opposed to all 52 if we did a linear search.

More generally, if the array contains $n$ elements, binary search requires $\log_2 n$ comparisons, and linear search requires $n$. In turn, the run-time of binary search is \textit{O($\log_2 n$)}
